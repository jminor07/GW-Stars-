      program padask
C     like wwwask, but modified for use with Padova2011 isochrones
      integer     histpar
...
C            : 
C            : Finally, the "Po__vanilla" string is a directory name
...
C !!! ??? note that the abundance scale could be expanded to -2.23 to 0.62
C        in this Padova-enabled version
...
C      dirname = '/space/www/dial/exportpad11/'//dirname
         nchar = 34
         print*, dirname
...
C     get and store the interpolated population models
      do i=1,npop
         ...
         call getmodel11(zin,agein,dirname,nchar,
...
C     open some output files
      open(unit=45,file='padask.output',status='unknown',
     z access='append')
      t = ctime(time())
...
C     form indices and output them to a file
      ...
      open(unit=56,file='padask.200',status='unknown',access='append')
...
C     high-res flux
      ...
      open(unit=46,file='padask.hspectrum',status='unknown',
     z access='append')
      do j=1,14000
         wv = 3000.0 + 0.5*real(j-1)
      ...
      open(unit=47,file='padask.mags',status='unknown',access='append')
      write(47,'(150a9)') (maglabels(k),k=1,nclrs)
      write(47,'(150f9.3)') (mags(k),k=1,nclrs)
      close(47)

      open(unit=48,file='padask.lbars',status='unknown',access='append')
